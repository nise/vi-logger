...
define(['jquery'], function ($) {
    ...
    Log.prototype = {
        ...
        options: {
            ...
            logger_selector: 'log' // if output_type === 2
        },
        ...
        output: function (logEntry) {
            // output
            if (this.options.output_type === 0) { // console
                console.log(logEntry);
            } else if (this.options.output_type === 1) { // Server
                this.sendLog(logEntry);
            } else if (this.options.output_type === 2) { // DOM
                var output = document.getElementById( this.options.logger_selector );
                if( output ){
                    //output.value += JSON.stringify(logEntry) + '&#013;&#010;';
                    //output.value += Object.values(logEntry) + '&#013;&#010;';
                    if (
                        ...
                        || (document.getElementById('logclickstream').checked && logEntry.action === 'clickstream')
                    ) {
