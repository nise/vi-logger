define(["jquery", "lib/vi2/vi2.main"], function ($, Vi2) {
  ...
  function computeWatchTime() {
    ...
    if (res["timeupdate"] !== undefined) {
      var epsilon2 = clickstream_tolerance.value,
      ...
      document.getElementById("resulttimeupdate").innerHTML = (
        timeupdate_watching_time / 1000
      ).toFixed(1);
    }

    // compute watching time using segments
    if (res["playback"] !== undefined) {
      document.getElementById("resultsegment").innerHTML =
        res["playback"].length * check_segments_length.value;
    } else {
    ...
    if (res["heartbeat"] !== undefined) {
      ...
      document.getElementById("resultheartbeat").innerHTML = (
        heartbeat_watching_time / 1000
      ).toFixed(1);
    }

    // compute watching time using clickstreams
    if (res["clickstream"] !== undefined) {
      var clickstream_watching_time = 0,
      ...
      document.getElementById("resultclickstream").innerHTML = (
        clickstream_watching_time / 1000
      ).toFixed(1);
    }
  }
});
